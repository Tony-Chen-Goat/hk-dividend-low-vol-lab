...
| `monthly_features` | `model_name, month_end, symbol` | Yahoo 基础10因子或完整13因子的原值、缩尾值、得分、贡献、总分、覆盖率与质量标记（JSON 列）；两种模式独立存储 |
| `forward_returns` | `month_end, symbol` | 因子月与下一有效月末之间的复权价格收益 |
| `backtest_holdings` | `experiment_id, month_end, symbol` | 每月目标权重、实际收益与贡献 |
| `experiments` | `experiment_id` | 因子模式、参数、样本窗口、风险约束、样本外指标与得分 |
