...
from app.config import DEFAULT_DB_PATH, FACTOR_GROUPS, FACTOR_LABELS, FACTOR_WEIGHTS
...
for group, factors in FACTOR_GROUPS.items():
    with st.expander(group, expanded=True):
        ...
        for index, factor in enumerate(factors):
            weights[factor] = columns[index % len(columns)].number_input(FACTOR_LABELS[factor], 0.0, 1.0, FACTOR_WEIGHTS[factor], 0.01, format="%.2f", key=f"weight_{factor}")
...
if st.button("计算并保存月度因子", type="primary", disabled=not valid):
    ...
    try:
        panel = compute_and_store_features(DEFAULT_DB_PATH, weights, progress)
        if panel.empty:
...
if panel.empty:
    ...
    st.plotly_chart(factor_correlation_chart(latest, list(FACTOR_WEIGHTS)), use_container_width=True)
st.markdown("#### 公式边界")
