...
def database_quality_snapshot(path: str | Path = DEFAULT_DB_PATH) -> dict:
    ...
    with connect(path) as conn:
        latest, price_symbols = conn.execute(
            "SELECT MAX(trade_date), COUNT(DISTINCT symbol) FROM daily_prices"
        ).fetchone()
        symbols = conn.execute("SELECT COUNT(DISTINCT symbol) FROM security_master").fetchone()[0]
        dividend_symbols = conn.execute("SELECT COUNT(DISTINCT symbol) FROM dividends").fetchone()[0]
        ...
        effective_rows, ended_rows = conn.execute(
            """SELECT SUM(CASE WHEN effective_date IS NOT NULL THEN 1 ELSE 0 END),
